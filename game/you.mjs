...
function You() {
	...
	this.id = -1
	this.type = 0
	this.rank = function () { return Math.ceil(50000000000000 * (1 / (((this.agl + this.str + (this.eqp[0].str) + this.spd + this.int) ** 2) / Math.sqrt((this.agl + this.str + this.int + (this.spd / this.lvl)) * 512 / (this.luck * .1 + 1))))) }; this.rnk = 0
	this.lvl = 1
	this.exp = 0
	this.expnext = function () { return this.lvl * ((this.lvl * 2) ** 2) + (this.lvl ** 2) }; this.expnext_t = this.expnext()
	this.exp_t = 1
	this.efficiency = function () { let g = skl.fmn.use(); g = g >= .6 ? .6 : g; let e = (.8 - g) * this.sat / this.satmax + (.2 + g) + this.mods.sbonus; return e < 0 ? 0 : e }
	...
	this.onDeath = function (killer) {
		if (this.res.death < 1 && random() >= this.res.death) { msg('You avoid death...', 'lightgrey'); this.hp = Math.ceil(this.hpmax * .1) } else {
			...
			this.sat / this.satmax > .3 ? giveSkExp(skl.dth, killer.rnk * 10 + 1) : giveSkExp(skl.dth, killer.rnk + 1)
	...
	this.stat_r = function () {
		...
		} dom.d6.update(); update_db(); if (this.hp > this.hpmax) this.hp = this.hpmax; dom.d5_1_1.update()
	}
	this.eqp[0].ctype = 2
	this.ai = function () {}
}